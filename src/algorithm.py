...
from src.utils import random_hypercube_samples, constrain_points, accumulate_immediate_regret, plot_immediate_regret
from .priors import *
...
class AcquisitionAlgorithm(object):
        ...
        for i in range(0, self.n_iter):
            ...
            if do_plot:
                ...
                if i is not 0 and i % 20 == 0:
                    ir = accumulate_immediate_regret(self.model.Y, self.f_opt)
    ...
    def plot_prediction(self, x_new=None, bounds=None, save_dist=None, plot_predictions=True, plot_embedded_subspace=False):
        ...
        if embedded_dims == 2:
            ...
            X = self.model.X
            idx = (bounds[0, 0] <= X[:, 0]) & (X[:, 0] <= bounds[0, 1]) & (bounds[1, 0] <= X[:, 1]) & (
                        X[:, 1] <= bounds[1, 1])
            X0 = X[idx, 0]
            X1 = X[idx, 1]
            ...
            plt.subplot(1, 2, 1)
            self.model.plot_prediction(X_line, Y_line, X_embedding=X_line_embedded, x_new=x_new,
